...
static int local_fsm(char c)
{
    ...
    case fsm_clearspaces:
        ...
        case '/':	
            ...
            break;
        case '(':	
            appen_ex_item(ei_ob, NO_PRIO, 0);		
            break;
        case ')':	
            appen_ex_item(ei_cb, TOP_PRIO, 0);	
            break;
        case '+':	
            appen_ex_item(ei_add, ADDITIVE_PRIO, 0);	
            break;
        case '-':	
            appen_ex_item(ei_sub, ADDITIVE_PRIO, 0);	
            break;
        default:
            printf("TOOOOODO\n");
        }
...
static int parse_local_ex()
{
    ...
    if (!op_queue_tail || value_queue_head /*|| op_queue_tail->*/)
        status = -944;
    else
    {
...
static void do_operation(ex_item_t * i)
{
    ...
    case ei_sub:	
        ...
        break;
    default: 
        throw "FIX ME ASAP in preproc.cc : void do_operation()";
    }
    free(op_queue_tail);
    op_queue_tail = pv2;
    pv2->prev = 0;
