...
static static_data_t * TryEnumsAndConstants(type_t * type, Code::lexem_node_t * node)
{
	if (type->prop == type->typedef_type)
	{
		typedef_t * def = (typedef_t*)type;
		return TryEnumsAndConstants(def->type, node);
	}
	else if (type->prop == type->enumerated_type)
	{
...
static static_data_t * ÑheckLexemData(type_t * type, Code::lexem_node_t * node)
{
	...
	case lt_openblock:
	{
			...
			for (auto  statement : *node->statements)
			{
				for (auto lexem : statement)
				{
					if (lexem.lexem == lt_comma)
						continue;
					static_data_t * res = ÑheckLexemData(array->child_type, new Code::lexem_node_t(lexem));
		...
		{
			typedef_t * def = (typedef_t*)type;
			ok = ÑheckLexemData(def->type, node);
			break;
		}
	...
	for (auto statement : *encoded_data)
	{
		...
		for (auto sequence : statement)
		{
			...
			case lt_comma:
				...
				continue;
			default:
				static_data_t * res = ÑheckLexemData((*var)->type, &sequence);
				(*ok->nested).push_back(res);
				continue;
			}
		}
