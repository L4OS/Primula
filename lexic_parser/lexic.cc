...
lexem_type_t translate_builtin_types()
{
	...
	for (int k = 0; k < lex_counter; k++) switch (lex_queue[k])
	{
	case lt_lex_bool:
		return (lex_counter == 1) ? lt_type_bool : (lexem_type_t)-2000;
	case lt_lex_char:		c_char++;		break;
	case lt_lex_double:		c_double++;		break;
	case lt_lex_float:		c_float++;		break;
	case lt_lex_short:		c_short++;		break;
	case lt_lex_int:		c_int++;		break;
	case lt_lex_long:		c_long++;		break;
	case lt_lex_signed:		c_signed++;		break;
	case lt_lex_unsigned:	c_unsigned++;	break;
	}
	...
	if (c_double > 0)
	{
		...
		if (check_error || c_double > 1 || c_long > 1) return (lexem_type_t)-2002;
		return lt_type_double;
	}
	else if (c_float > 0)
	{
		check_error = c_signed + c_unsigned + c_char + c_short + c_int + c_long;
		if (check_error || c_float > 1) return (lexem_type_t)-2003;
		return lt_type_float;
	}
	else if (c_char > 0)
	{
		check_error = c_short + c_int + c_long;
		if (check_error || c_char > 1) return (lexem_type_t)-2004;
		return c_unsigned > 0 ? lt_type_uchar : lt_type_char;
	}
	else if (c_short > 0)
	{
		if (c_long || c_short > 1) return (lexem_type_t)-2005;
		return c_unsigned > 0 ? lt_type_ushort : lt_type_short;
	}
	else if (c_long > 0)
	{
		if (c_long == 1) return c_unsigned > 0 ? lt_type_ulong : lt_type_long;
		if (c_long == 2) return c_unsigned > 0 ? lt_type_ullong : lt_type_llong;
		return (lexem_type_t)-2006;
	}
	else if (c_int > 1) return (lexem_type_t)-2007;
...
void append_lexem(lexem_type_t type)
{
	...
	if (mode == om_debug) {
		char * s = l->lexem;
		if (type == lt_word && !l->lexem) s = (char*) "_STATE_ERROR_";
...
static int global_state_machine(unsigned char ch, flow_control_t	*	flow)
{
	...
	case gs_expression:
		if (ch == '/') {
			flow->global_state = gs_parse_slash; return 0;
		}
		if (ch == 0x0a) {
			flow->line_number++; return 0;
		}
		if (ch == '#') {
	...
	case gs_delimiter:
		...
		flow->global_state = gs_lexem;
		if (isalpha(ch) || ch == '_') goto reparse;
		if (ch == '{') { append_lexem(lt_openblock);	return 0; }
		if (ch == '}') { append_lexem(lt_closeblock);	return 0; }
		if (ch == '(') { append_lexem(lt_openbraket);	return 0; }
		if (ch == ')') { append_lexem(lt_closebraket);	return 0; }
		if (ch == ';') { append_lexem(lt_semicolon);	return 0; }
		if (ch == '[') { append_lexem(lt_openindex);	return 0; }
		if (ch == ']') { append_lexem(lt_closeindex);	return 0; }
		if (ch == ',') { append_lexem(lt_comma);		return 0; }
		if (ch == '?') { append_lexem(lt_quest);		return 0; }
		if (ch == '~') { append_lexem(lt_tilde);		return 0; }
		if (isdigit(ch)) {
			number_type = nt_decimal;
			current_lexem[current_lexem_size++] = ch;
			flow->global_state = gs_numeric;
		...
		if (ch == ':') { flow->global_state = gs_colon;			return 0; }	// append_lexem( lt_colon );		return 0;	}
		if (ch == '*') { flow->global_state = gs_mul;			return 0; }
		if (ch == '=') { flow->global_state = gs_equally;		return 0; }
		if (ch == '+') { flow->global_state = gs_plus;			return 0; }
		if (ch == '-') { flow->global_state = gs_minus;			return 0; }
		if (ch == '"') { flow->global_state = gs_string;			return 0; }
		if (ch == '&') { flow->global_state = gs_and;			return 0; }
		if (ch == '^') { flow->global_state = gs_xor;			return 0; }
		if (ch == '|') { flow->global_state = gs_or;				return 0; }
		if (ch == '>') { flow->global_state = gs_more;			return 0; }
		if (ch == '<') { flow->global_state = gs_less;			return 0; }
		if (ch == '/') { flow->global_state = gs_parse_slash; 	return 0; }
		if (ch == '%') { flow->global_state = gs_parse_rest; 	return 0; }	// append_lexem( lt_rest ); 		return 0; 	}
		if (ch == '!') { flow->global_state = gs_exclamation; 	return 0; }
		if (ch == '\'') { flow->global_state = gs_apost_begin; 	return 0; }
		if (ch == '.') { flow->global_state = gs_dot;			return 0; }
		if (ch == '@') { flow->global_state = gs_linear_comment; return 0; }
	...
	case gs_apost_begin:
		if (ch == '\'' && !current_lexem_size) return -7; // empty character constant			
	...
	case gs_apost_end:
		if (ch != '\'') return -8; // too many characters in constant
	...
	case gs_equally:
		flow->global_state = gs_lexem;
		if (ch != '=') { append_lexem(lt_set);	goto reparse; }
		append_lexem(lt_equally);
		return 0;

	case gs_mul:
		flow->global_state = gs_lexem;
		if (ch != '=') { append_lexem(lt_mul);		goto reparse; }
		append_lexem(lt_mul_and_set);
		return 0;

	case gs_colon:
		flow->global_state = gs_lexem;
		if (ch != ':') { append_lexem(lt_colon);		goto reparse; }
		append_lexem(lt_namescope);
		return 0;

	case gs_exclamation:
		flow->global_state = gs_lexem;
		if (ch != '=') { append_lexem(lt_logical_not);		goto reparse; }
	...
	case gs_plus:
		flow->global_state = gs_lexem;
		switch (ch) {
		case '+': append_lexem(lt_inc); break;
		case '=': append_lexem(lt_add_and_set); break;
		default:  append_lexem(lt_add); goto reparse;
		}
		return 0;
	case gs_minus:
		flow->global_state = gs_lexem;
		switch (ch) {
		case '-': append_lexem(lt_dec); break;
		case '=': append_lexem(lt_sub_and_set); break;
		case '>': flow->global_state = gs_check_ptr_to_member; break;
		default:  append_lexem(lt_sub); goto reparse;
		}
		return 0;
	...
	case gs_more:
		flow->global_state = gs_lexem;
		switch (ch) {
		case '>':	append_lexem(lt_shift_right); break;
		case '=':	append_lexem(lt_more_eq);		break;
		default:	append_lexem(lt_more);		goto reparse;
		}
		return 0;
	...
	case gs_less:
		flow->global_state = gs_lexem;
		switch (ch) {
		case '<':	append_lexem(lt_shift_left);	break;
		case '=':	append_lexem(lt_less_eq);		break;
		default:	append_lexem(lt_less);		goto reparse;
		}
		return 0;
	...
	case gs_numeric:
	{
		...
		switch (number_type)
		{
		case nt_decimal:		append_lexem(lt_integer);	break;
		case nt_hexdecimal:		append_lexem(lt_integer);	break;
		case nt_float:			append_lexem(lt_floatnumber);	break;
		}
	}
	goto reparse;
	...
	case gs_checking_comment:
		switch (ch) {
		case '/':	flow->global_state = gs_lexem;	break;
		case 0x0a:	flow->line_number++; // Здесь break ни в коем случе не нужен
		case '*':	break;
		default:	flow->global_state = gs_inside_comment;
		}
		break;

	case gs_linear_comment:
		if (ch == 0x0d) {
			flow->global_state = gs_caret_feed; return 0;
		}
		if (ch == 0x0a) {
			flow->line_number++; flow->global_state = gs_expression; return 0;
		}
		break;
	...
	case gs_prep_check_comment:
		switch (ch) {
		case '/':	flow->global_state = gs_do_preprocessor;	break;
		case 0x0a:	flow->line_number++; // Здесь break ни в коем случе не нужен
		case '*':	break;
		default:	flow->global_state = gs_prep_block_comment;
		}
		break;
		...
		// Keep parser consistent on # inside comments
	{
		...
		if (ch == 0x0a) flow->line_number++;
		if (status_quo == normal_skip) {
			if (ch == '#') {
				flow->global_state = gs_do_preprocessor;
				break;
			}
			if (ch == '/') status_quo = is_comment;
			break;
		}
		if (status_quo == is_comment) {
			switch (ch) {
			case '/':	status_quo = line_comment;	break;
			case '*':	status_quo = block_comment;	break;
			default:	status_quo = normal_skip;	break;
			}
			break;
		}
		if (status_quo == line_comment) {
			if (ch == 0x0a) status_quo = normal_skip;
			break;
		}
		if (status_quo == block_comment) {
			if (ch == '*') status_quo = term_comment;
			break;
		}
		if (status_quo == term_comment) {
			if (ch == '/') status_quo = normal_skip;
			else status_quo = block_comment;
		}
	}
	break;
